...
Expect<void> checkLabel(std::string_view Label,
  ...
  if (!ComponentName::isKebabString(Label)) {
...
Validator::validate(const AST::Component::CoreDefType &DType) noexcept {
  ...
  auto *Shape = CompCtx.newCoreShape();
  EXPECTED_TRY(validateModuleType(DType.getModuleType(), *Shape));
...
Validator::validate(const AST::Component::DefType &DType) noexcept {
  if (DType.isDefValType()) {
    ...
    auto *Shape = CompCtx.newComponentShape();
    EXPECTED_TRY(validateInstanceType(DType.getInstanceType(), *Shape));
    auto &S = CompCtx.top();
    S.Types.push_back({&DType, &S, nullptr, Shape, nullptr, {}, {}});
  } else if (DType.isComponentType()) {
    auto *Shape = CompCtx.newComponentShape();
    EXPECTED_TRY(validateComponentType(DType.getComponentType(), *Shape));
    auto &S = CompCtx.top();
  ...
  {
    auto &S = CompCtx.top();
    ComponentContext::ExternInfo Probe;
    Probe.K = ComponentContext::ExternInfo::Kind::Type;
    Probe.Type = S.Types.back();
    EXPECTED_TRY(CompCtx.checkTypeSize(CompCtx.sizeOfExtern(Probe)));
    EXPECTED_TRY(CompCtx.checkTypeDepth(CompCtx.depthOfExtern(Probe)));
  }
  return {};
}
...
Expect<void>
Validator::validateModuleType(Span<const AST::Component::CoreModuleDecl> Decls,
                              ComponentContext::CoreShape &Shape) noexcept {
  CompCtx.enterScope(ComponentContext::Scope::Kind::ModuleType);
  for (const auto &Decl : Decls) {
    if (Decl.isImport()) {
      const auto &Imp = Decl.getImport();
      EXPECTED_TRY(auto Ext, validate(Imp.getImportDesc()));
      for (const auto &[ModName, Name, Prev] : Shape.Imports) {
      ...
      Shape.Imports.emplace_back(std::string(Imp.getModuleName()),
      ...
      if (!Shape.Exports.emplace(std::string(Exp.getName()), Ext).second) {
...
Expect<void> Validator::validateInstanceType(
    const AST::Component::InstanceType &IT,
    ComponentContext::ComponentShape &Shape) noexcept {
  auto &S = CompCtx.enterScope(ComponentContext::Scope::Kind::InstanceType);
  Shape.DeclScope = &S;
  for (const auto &Decl : IT.getDecl()) {
    const auto Before = Shape.Exports.size();
    EXPECTED_TRY(validate(Decl, Shape.Exports));
    if (Shape.Exports.size() != Before && Decl.isExportDecl()) {
      Shape.ExportOrder.emplace_back(Decl.getExport().getName());
    }
  }
  CompCtx.exitScope();
  return {};
...
Expect<void> Validator::validateComponentType(
    const AST::Component::ComponentType &CT,
    ComponentContext::ComponentShape &Shape) noexcept {
  auto &S = CompCtx.enterScope(ComponentContext::Scope::Kind::ComponentType);
  Shape.DeclScope = &S;
  for (const auto &Decl : CT.getDecl()) {
    EXPECTED_TRY(validate(Decl, Shape));
  }
  CompCtx.exitScope();
  return {};
}
...
Validator::validate(const AST::Component::InstanceDecl &Decl,
  ...
  if (Decl.isExportDecl()) {
    ...
    EXPECTED_TRY(ComponentName CN,
                 CompCtx.parseExternName(ED.getName(), false));
    const auto DeclClash = CompCtx.addUniqueName(CompCtx.top().ExportNames,
                                                 CompCtx.makeNameRecord(CN));
    ...
    EXPECTED_TRY(CompCtx.checkImplements(
        CN, ED.getImplements(),
...
Validator::validate(const AST::Component::ComponentDecl &Decl,
  ...
  if (Decl.isImportDecl()) {
    ...
    EXPECTED_TRY(auto Resolved, validate(ID.getExternDesc(), true));
    EXPECTED_TRY(auto Ext, CompCtx.defineImport(ID.getName(), Resolved,
                                                ID.getImplements()));
    Shape.Imports.emplace_back(std::string(ID.getName()), Ext);
    return {};
  }
  return validate(Decl.getInstance(), Shape.Exports);
}
...
void ComponentContext::defineExtern(
...
ComponentContext::parseExternName(std::string_view Name,
  ...
  if (CN.getKind() == ComponentName::Kind::Invalid) {
    spdlog::error(ErrCode::Value::InvalidExternName);
    spdlog::error("    {} name '{}' is not a valid extern name."sv, Position,
                  Name);
    return Unexpect(ErrCode::Value::InvalidExternName);
  ...
  if (!IsImport && (CN.getKind() == ComponentName::Kind::LockedDep ||
                    CN.getKind() == ComponentName::Kind::UnlockedDep ||
                    CN.getKind() == ComponentName::Kind::Url ||
                    CN.getKind() == ComponentName::Kind::Integrity)) {
...
Expect<void> ComponentContext::checkImplements(const ComponentName &CN,
  ...
  for (const auto &I : Impls) {
    ...
    if (Parsed->getKind() != ComponentName::Kind::InterfaceType) {
  ...
  if (CN.getKind() != ComponentName::Kind::Label &&
      CN.getKind() != ComponentName::Kind::Constructor &&
      CN.getKind() != ComponentName::Kind::Method &&
      CN.getKind() != ComponentName::Kind::Static) {
...
Expect<ComponentContext::ExternInfo>
ComponentContext::defineImport(std::string_view Name,
  ...
  if (Info.K == ExternInfo::Kind::Instance) {
    Info.Shape = freshenDeclaredResources(Info.Shape, true);
  }
  EXPECTED_TRY(ComponentName CN, parseExternName(Name, true));
  EXPECTED_TRY(
      checkImplements(CN, Impls, Info.K == ExternInfo::Kind::Instance));
  const auto Clash = addUniqueName(top().ImportNames, makeNameRecord(CN));
  if (Clash != NameClash::None) {
    ErrCode::Value Code;
    if (top().K != Scope::Kind::Component) {
      Code = Clash == NameClash::Duplicate
                 ? ErrCode::Value::ComponentNameConflict
...
Expect<ComponentName>
ComponentContext::registerExportName(std::string_view Name, bool IsInstance,
                                     Span<const std::string> Impls) noexcept {
  EXPECTED_TRY(ComponentName CN, parseExternName(Name, false));
  EXPECTED_TRY(checkImplements(CN, Impls, IsInstance));
  const auto Clash = addUniqueName(top().ExportNames, makeNameRecord(CN));
  if (Clash != NameClash::None) {
    const auto Code = top().K != Scope::Kind::Component
                          ? ErrCode::Value::ComponentDeclExportNameConflict
                          : ErrCode::Value::ComponentNameConflict;
    spdlog::error(Code);
...
Expect<ComponentContext::ExternInfo> ComponentContext::defineExport(
    const ComponentName &CN, const ExternInfo &Inferred,
    const std::optional<ExternInfo> &Ascribed) noexcept {
  ExternInfo Result = Inferred;
  if (Ascribed.has_value()) {
    ResourceSubst Subst;
    if (!matchExtern(Inferred, *Ascribed, Subst)) {
  ...
  if (Result.K == ExternInfo::Kind::Type) {
    Result.Type.NameId = newNameId();
  }
  if (Result.K == ExternInfo::Kind::Instance) {
...
Expect<void>
ComponentContext::checkAnnotatedName(const ComponentName &Name,
                                     const ComponentContext::ExternInfo &Info,
                                     bool IsImport) noexcept {
  const auto Kind = Name.getKind();
  if (Kind != ComponentName::Kind::Constructor &&
      Kind != ComponentName::Kind::Method &&
      Kind != ComponentName::Kind::Static) {
    return {};
  }
  ...
  } else if (Kind == ComponentName::Kind::Method) {
...
void ComponentContext::recordResourceLabel(
    const ComponentName &Name, const ComponentContext::ExternInfo &Info,
    bool IsImport) noexcept {
  if (Name.getKind() == ComponentName::Kind::Label &&
      Info.K == ComponentContext::ExternInfo::Kind::Type &&
...
bool ComponentContext::isValTypeIntroduced(
    const ComponentContext::QualValType &Q, bool IsImport) noexcept {
...
bool ComponentContext::isTypeEntryIntroduced(
    const ComponentContext::TypeEntry &E, bool IsImport) noexcept {
  auto &S = top();
...
bool ComponentContext::areInnerTypesIntroduced(
    const ComponentContext::TypeEntry &E, bool IsImport) noexcept {
  if (E.ResourceId.has_value()) {
    return true;
  }
  ...
  if (D.isOwnTy() || D.isBorrowTy()) {
    const auto &NamedRes =
        IsImport ? top().ImportNamedResources : top().ExportNamedResources;
    const uint32_t Idx = D.isOwnTy() ? D.getOwn().Idx : D.getBorrow().Idx;
    const auto *Res = E.Home->getType(Idx);
    if (Res == nullptr || !Res->ResourceId.has_value()) {
      return true;
...
bool ComponentContext::introduceExternTypes(
    const ComponentContext::ExternInfo &Info, bool IsImport) noexcept {
  auto &S = top();
  switch (Info.K) {
  case ExternInfo::Kind::CoreModule:
  case ExternInfo::Kind::Component:
...
Expect<void>
ComponentContext::checkNamedTypesRule(const ComponentContext::ExternInfo &Info,
                                      bool IsImport) noexcept {
  // Instance-type declarations do not enforce the named-types rule; their
  // shapes are re-checked when imported or exported.
  if (top().K == ComponentContext::Scope::Kind::InstanceType) {
    return {};
  }
...
Expect<const ComponentContext::CoreShape *>
ComponentContext::buildCoreShape(const AST::Module &Mod) noexcept {
  auto *Info = newCoreShape();
