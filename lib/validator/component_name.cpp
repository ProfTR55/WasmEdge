...
// word           ::= [0-9a-z]+
// acronym        ::= [0-9A-Z]+
bool ComponentName::isKebabString(std::string_view Input) noexcept {
...
Expect<void> ComponentName::parse(std::string_view Name) noexcept {
  OriName = Name;
  NoTagName = {};
...
bool ComponentName::tryRead(std::string_view Prefix) noexcept {
...
bool ComponentName::readUntil(char Delim, std::string_view &Output) noexcept {
...
std::string_view ComponentName::readLabelChars() noexcept {
  size_t Pos = 0;
  while (Pos < Rest.size() && (isalnum(Rest[Pos]) || Rest[Pos] == '-')) {
    Pos++;
  }
...
//             | '[method]' <label> '.' <label>
//             | '[static]' <label> '.' <label>
Expect<void> ComponentName::parsePlainName() noexcept {
  ...
  if (tryRead("[async]"sv) || tryRead("[async method]"sv) ||
      tryRead("[async static]"sv)) {
    spdlog::error(ErrCode::Value::ComponentInvalidName);
    spdlog::error(
        "    Component name: async annotations are not supported yet"sv);
...
//                | '@{' <verlower> ' ' <verupper> '}'
Expect<void> ComponentName::parseUnlockedDep() noexcept {
  if (!tryRead("<"sv)) {
...
Expect<void> ComponentName::parseLockedDep() noexcept {
  if (!tryRead("<"sv)) {
    spdlog::error(ErrCode::Value::NameExpectedOpenAngle);
...
// nonbrackets ::= [^<>]*
Expect<void> ComponentName::parseUrlName() noexcept {
  if (!tryRead("<"sv)) {
    spdlog::error(ErrCode::Value::NameExpectedOpenAngle);
...
Expect<void> ComponentName::parseHashName() noexcept {
  if (!tryRead("<"sv)) {
    spdlog::error(ErrCode::Value::NameExpectedOpenAngle);
...
Expect<void> ComponentName::parseInterfaceName() noexcept {
...
Expect<ComponentName::PkgPath>
ComponentName::parsePkgPath(std::string_view StopChars) noexcept {
  size_t ColonPos = Rest.find(':');
...
Expect<std::string_view> ComponentName::parseIntegrityBody() noexcept {
...
// Parses the optional ',' <hashname> suffix; an exhausted input yields no
// integrity metadata.
Expect<std::string_view> ComponentName::parseIntegritySuffix() noexcept {
  if (Rest.empty()) {
...
// Validates a namespace or package label: a label that is not even
// kebab-shaped (e.g. `WaSi`) is "not in kebab case", while a well-formed kebab
// label carrying uppercase (e.g. `A`) is "not lowercase".
Expect<void>
ComponentName::checkWordsLabel(std::string_view Label,
...
Expect<void>
ComponentName::checkVersionRange(std::string_view Body) const noexcept {
...
//                | '0.' [1-9] [0-9]*
//                | '0.0.' [1-9] [0-9]*
bool ComponentName::isCanonVersion(std::string_view V) const noexcept {
...
// MAJOR.MINOR.PATCH[-prerelease][+build] per semver.org 2.0. The error carries
// the granular code matching the Rust `semver` crate wasm-tools relies on, but
// is not logged: an interface version reports it as-is, while a locked-dep or
// version-range bound reports NameNotValidSemver instead.
Expect<void> ComponentName::scanSemver(std::string_view V) const noexcept {
  if (V.empty()) {
...
Expect<void>
ComponentName::scanSemverIdentifiers(std::string_view Idents,
...
Expect<void>
ComponentName::checkIntegrityMetadata(std::string_view Input) const noexcept {
