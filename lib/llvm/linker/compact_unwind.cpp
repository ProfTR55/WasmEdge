...
Expect<uint64_t> machOUnwindInfoSizeForCounts(uint64_t CommonEncodingCount,
                                              uint64_t PageCount,
                                              uint64_t LSDACount) {
  ...
  if (!AddProduct(CommonEncodingCount, 4) || PageCount == UINT64_MAX ||
      !AddProduct(PageCount + 1, UnwindIndexSize) ||
      !AddProduct(LSDACount, UnwindLSDASize) ||
      !AddProduct(PageCount, UnwindPageSize) || Result > UINT32_MAX)
    return Unexpect(ErrCode::Value::IllegalPath);
  return Result;
}
...
Expect<uint64_t> machOUnwindInfoSize(const LinkGraph &Graph) {
  ...
  return Internal::machOUnwindInfoSizeForCounts(
      Layout.ReservedCommonCount, Layout.ReservedPageCount, Layout.LSDACount);
}
