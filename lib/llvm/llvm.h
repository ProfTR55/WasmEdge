...
public:
  ...
  std::string getInlineAsm() const {
    size_t Size = 0;
    const char *Data = LLVMGetModuleInlineAsm(Ref, &Size);
    return std::string(Data, Size);
  }
  void setInlineAsm(std::string_view Value) noexcept {
#if LLVM_VERSION_MAJOR >= 12
    LLVMSetModuleInlineAsm2(Ref, Value.data(), Value.size());
#else
    const std::string Copy(Value);
    LLVMSetModuleInlineAsm(Ref, Copy.c_str());
#endif
  }
...
#undef DECLARE_VALUE_CHECK
  ...
  void setDSOLocal(bool Local) noexcept;
  ...
  void eliminateUnreachableBlocks() noexcept;
...
public:
  ...
  bool isBSS() const noexcept;
  bool isPData() const noexcept;
  bool isEHFrame() const noexcept;
